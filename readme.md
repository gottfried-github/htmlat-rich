...
One issue here is that any tags get escaped as a consequence of using `createTextNode` in the renderer and become invisible to sanitizer (e.g., see `Sanitization` in `htmlat-nodejs`).
